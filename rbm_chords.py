...
num_epochs = 200 #number of times the dataset is passed through traininh
batch_size = 100
lr = 0.005 #learning rate

x = tf.compat.v1.placeholder(tf.float32, [None, n_visible], name="x") #gateway for inputting data in our model
W = tf.Variable(tf.random.normal([n_visible, n_hidden], mean=0.0, stddev=0.01), name="W")
bh = tf.Variable(tf.zeros([1, n_hidden], tf.float32, name="bh")) #bias variable for hidden layer
...
x_sample = gibbs_sample(1) #sample visible layer state
h = sample(tf.sigmoid(tf.matmul(x, W) + bh)) #compute probab for hidden layer
